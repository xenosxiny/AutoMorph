...
def fast_keypoints(image, image_draw, roots, tail=15, save_gif=False): 
    '''
...
def run(path, result_path):
    # path = './src/Keypoints_Detection/DRIVE/training/1st_manual/'
    ...
    for file_name in file_list:
        ...
        image_draw, mask, plate, angle_map, keypoints = fast_keypoints(image_init, image_draw, [roots], save_gif=False)
        ...
        # cv2.imwrite(result_path + f'plot/{file_name}_attention_map.png', gradient_flow)
        ...
        image_draw, mask, plate, angle_map, keypoints = fast_keypoints(image_circle, image_draw, [roots], tail=15, save_gif=False)
        
        
        result = cv2.add(plate, mask)

        # plt.imshow(result)
        # cv2.imwrite(result_path + f'plot/{file_name}_keypoints.png', cv2.cvtColor(result, cv2.COLOR_BGR2RGB))
        
        # plt.imshow(image_draw)
        # cv2.imwrite(result_path + f'plot/{file_name}_keypoints_draw.png', cv2.cvtColor(image_draw, cv2.COLOR_BGR2RGB))
        
        # plt.imshow(angle_map)
        # cv2.imwrite(result_path + f'plot/{file_name}_angle_map.png', cv2.cvtColor(angle_map, cv2.COLOR_BGR2RGB))
        ...
        # cv2.imwrite(result_path + f'plot/{file_name}_angle_draw.png', cv2.cvtColor(angle_draw, cv2.COLOR_BGR2RGB))
        angle_list_dim = np.array(angle_list, dtype=object).ndim
        if angle_list_dim == 2:
        ...
        avg_angle_list[file_name] = avg_angle
    ...
    with open(result_path + f'ours_{date_time_string}.json', 'w') as f:
        json.dump(RAND, f)

    df = pd.DataFrame(avg_angle_list.items(), columns=['Name', 'Average Branch Angle'])
    df.to_csv(result_path + path.split('/')[-2] +  '_branch_angle.csv', index=False)


path_disc = f'{AUTOMORPH_DATA}/Results/M2/binary_vessel/disc_centred_binary_skeleton/'
path_macula = f'{AUTOMORPH_DATA}/Results/M2/binary_vessel/macular_centred_binary_skeleton/'

result_path = f'{AUTOMORPH_DATA}/Results/M4/'

if not os.path.exists(result_path):
    os.makedirs(result_path)

if not os.path.exists(result_path + 'plot/'):
    os.makedirs(result_path + 'plot/')
    
run(path_disc, result_path)
run(path_macula, result_path)

def merge_to_main_csv():
    main_csv_disc = pd.read_csv(f'{AUTOMORPH_DATA}/Results/M3/Disc_Features.csv')
    main_csv_macula = pd.read_csv(f'{AUTOMORPH_DATA}/Results/M3/Macular_Features.csv')
    
    disc_ba_csv = pd.read_csv(result_path + 'disc_centred_binary_skeleton_branch_angle.csv')
    macula_ba_csv = pd.read_csv(result_path + 'macular_centred_binary_skeleton_branch_angle.csv')
    
    merge_disc_df = pd.merge(main_csv_disc, disc_ba_csv, on = ['Name'], how='outer')
    merge_macula_df = pd.merge(main_csv_macula, macula_ba_csv, on = ['Name'], how='outer')
    merge_disc_df.to_csv(result_path + 'disc_features.csv', index=False)
    merge_macula_df.to_csv(result_path + 'macula_features.csv', index=False)
